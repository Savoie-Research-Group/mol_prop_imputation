...
def main(config):
    ...
    print(val_y.shape)
    # Setup DataLoader
    train_loader, val_loader = setup_data_loaders(train_x, train_y, val_x, val_y, config['batch_size'], config['fraction_dropped'])
...
def setup_data_loaders(train_x, train_y, val_x, val_y, batch_size, drop_frac):
    train_dataset = DynamicDropDataset(train_x, train_y, drop_probability=drop_frac)
    val_dataset = DynamicDropDataset(val_x, val_y, drop_probability=drop_frac)
...
def train_and_validate(config, model, optimizer, loss_function, loss_tracker, train_loader, val_loader, device):
    ...
    tr_curve(f"epochs, train_loss, val_loss, " + ", ".join(map(str, list(range(num_prop_rem)))), os.path.join(wk_dir, lc_fp))
