...
def main(config):
    ...
    train_x, train_smi, train_y, val_x, val_smi, val_y = preprocess_data(
        config
    )
    print(train_x.shape)
...
def setup_data_loaders(train_x, train_smi, train_y, val_x, val_smi, val_y, batch_size, drop_frac):
    train_dataset = DynamicSMILESDataset(
        train_x, train_smi, train_y, drop_probability=drop_frac)
    val_dataset = DynamicSMILESDataset(
        val_x, val_smi, val_y, drop_probability=drop_frac)
...
class DynamicSMILESDataset(Dataset):
    ...
    def __init__(self, inputs, smiles_list, targets, drop_probability=0.0,
    ...
    def __len__(self):
        return len(self.smiles_list)  # the first dimmension will be the smiles
    ...
    def __getitem__(self, idx):
        ...
        # Generate mask token randomly
        mask = torch.rand_like(
            inputs, dtype=torch.float) < self.drop_probability
        # Apply to property inputs
        inputs[mask] = self.missing_token

        # Tokenize the SMILES string (never masked)
...
def train_and_validate(config, model, optimizer, loss_function, loss_tracker, train_loader, val_loader, device):
    ...
    tr_curve(f"epochs, train_loss, val_loss, " + ", ".join(map(str,
             list(range(num_prop_rem)))), os.path.join(wk_dir, lc_fp))
    ...
    for epoch in range(current_epochs, current_epochs+epochs):
        ...
        train_loss = train_epoch(
            train_loader, model, optimizer, loss_function, device)
        val_loss, prop_mses = validate_epoch(
            val_loader, model, loss_function, loss_tracker, device)
        # prop_mses = prop_mses.squeeze().cpu().numpy()
        tr_curve(f"{epoch+1}, {train_loss}, {val_loss}, " + ", ".join(map(str,
                 prop_mses.squeeze(0).tolist())), os.path.join(wk_dir, lc_fp))
        prop_mses = prop_mses.squeeze().cpu().numpy()
        if isinstance(prop_mses, np.ndarray) and prop_mses.ndim == 0:
            prop_mses = [prop_mses.item()]
        print(
            f'Completed Epoch {epoch} with val loss {val_loss} and patience {current_patience}.')
        pat_test = False
        if epoch == 0:
            pat_test = True
            ...
            torch.save(model.state_dict(), os.path.join(
                wk_dir, md_sv_pre) + f'_cp_overall.pt')
            for i in range(num_prop_rem):
                torch.save(model.state_dict(), os.path.join(
                    wk_dir, md_sv_pre) + f'_cp_{i}.pt')
        else:
            for i in range(num_prop_rem):
                ...
                if prop_mses[i] < least_mse[i]:
                    pat_test = True
                    print(
                        f'        Saving index {i} at epoch {epoch}; loss = {prop_mses[i]}')
            ...
            if val_loss < least_val_loss:
                pat_test = True
                print(
                    f'        Saving overall at epoch {epoch}; loss = {val_loss}')
                least_val_loss = val_loss
                torch.save(model.state_dict(), os.path.join(
                    wk_dir, md_sv_pre) + f'_cp_overall.pt')
        # print(least_mse)
        ...
        print(
            f"    Lowest Overall: {least_val_loss:.3f}; lowest prop losses: {', '.join(f'{x:.3f}' for x in least_mse)}")
