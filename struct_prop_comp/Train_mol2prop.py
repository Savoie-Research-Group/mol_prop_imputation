...
def main(config):
    ...
    train_smi, train_y, val_smi, val_y = preprocess_data(config)
    ...
    # Setup DataLoader
    # exit()
    train_loader, val_loader = setup_data_loaders(train_smi, train_y, val_smi, val_y, config['batch_size'])
...
def preprocess_data(config):
    ...
    return train_smiles, train_y, val_smiles, val_y 
...
def train_epoch(dataloader, model, optimizer, loss_function, device):
    model.train()
    total_loss = 0
    for smiles, targets in tqdm(dataloader, desc="Training"):
        # print(inputs.shape)
        # print(smiles.shape)
        # print(targets.shape)
        # exit()
...
def validate_epoch(dataloader, model, loss_function, loss_tracker, device):
    ...
    with torch.no_grad():
        for smiles, targets in tqdm(dataloader, desc="Validation"):
            smiles = smiles.to(device)
            targets = targets.to(device)
            predictions = model(smiles)
            targets = targets.squeeze()
