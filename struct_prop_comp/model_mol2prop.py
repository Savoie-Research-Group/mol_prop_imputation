...
class Model(nn.Module):
    def __init__(self, pred_prop, include_quad=True, embed_dim=512, num_properties=22, dropout_prob=0.1, num_heads=16, num_feed_forward=4096, num_att_layers=4):
        ...
        self.property_decoders = nn.Linear(embed_dim, num_properties)
        ...
        self.chemberta_linear1 = nn.Linear(embed_dim, embed_dim)
        self.chemberta_linear2 = nn.Linear(embed_dim, embed_dim)
    ...
    def forward(self, smi):
        ...
        chembert_embed = F.relu(chembert_embed)
        chembert_embed = self.chemberta_linear1(chembert_embed)
        chembert_embed = F.relu(chembert_embed)
        chembert_embed = self.chemberta_linear2(chembert_embed)
        chembert_embed = F.relu(chembert_embed)
        props = self.property_decoders(chembert_embed)
        outputs = []
        for prop_idx in self.pred_prop:
            prop_output = props[:, prop_idx]  # Shape: (batch_size, 1)
